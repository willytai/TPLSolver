...
#ifdef XSTAR_ORIGINAL
SolverState ExactCoverSolver::X_star(int bfsIndex, bool recordPartialResult, int Depth) {
    ...
    cout << right << setw(6) << _dlx.cellcount() << " cells left in dlx, solving CC " << setw(4) << _component_id << '/' << _graph.numComponents()-1 << " Depth " << Depth << " vertex num " << _graph.size(_component_id) << endl;
    ...
    for (unsigned int i = 0; i < RelatedRow.size(); ++i) {
        ...
        ***********************************************************************/
        ...
        state = X_star(bfsIndex+1, recoredResultNextLevel, ++Depth);  // search for the next node by BFS order
...
SolverState ExactCoverSolver::X_star(int bfsIndex, bool recordPartialResult, int Depth) {
    ...
    Cell* TargetColumnCell = PriorityColumnCell;
    if (!PriorityColumnCell) {
        TargetColumnCell = _dlx.GetHeader()->right;
        assert(TargetColumnCell);
    }
    // maintain bfs order
    // else --bfsIndex;
